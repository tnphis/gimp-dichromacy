...
#Judging by the appearance, they used the same calibration as the authors of the Vischeck plugin
...
def process_brettel(p_region_data, anomaly, color_space, bpp, p_is_main_thread = False):
	i_total_len = len(p_region_data)
	#just repeat this code here, no big deal
	l_rgb_to_lms = []
	l_lms_to_rgb = []
	if color_space == 0:
	...
	try:
		...
		if model == 0:
			...
			for rgn in l_serialized_regions:
				l_results.append(o_pool.apply_async(process_brettel, [rgn, anomaly, color_space, layer.bpp]))

			l_new_region_collection[0] = process_brettel(o_riginal_serialized_region, anomaly, color_space, layer.bpp, True)
		elif model == 1:
...
register(
	...
	"Multimodel color blindness simulator. sRGB and Adobe RGB (Brettel only) color spaces, D65. Empirical (Brettel) and physiological models. This version is intended for use on all multicore (i.e. almost all modern) systems.  Unfortunately, rather slow due to slow math operations in Python and the necessity to perform pixel-level calculations. It should work on single core machines but will be slightly slower due to the serialization and deserialization of data.",
